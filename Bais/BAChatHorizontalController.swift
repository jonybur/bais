...
class BAChatHorizontalController: UIViewController, ASCollectionDataSource, ASCollectionDelegate {
    ...
    //MARK: - CollectionNode didSelectRowAt

    func collectionNode(_ collectionNode: ASCollectionNode, didSelectItemAt indexPath: IndexPath) {
        // taps friend (opens chat)
        let session = sessions[indexPath.item]
        self.superNavigationController?.pushViewController(BAChatController(with: session), animated: true)
    }
